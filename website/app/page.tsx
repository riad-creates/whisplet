    ...
    <>
      ...
      <main>
        ...
        <section className="install-section wrap" id="install">
          <div><p className="eyebrow">OPEN SOURCE. OPEN POSSIBILITIES.</p><h2>Make yourself<br /><em>heard.</em></h2><p>One command gets this early version ready.<br />No manual tool setup. No coding agent needed.</p><a className="button primary" href={`${repo}/blob/main/docs/INSTALL.md`}>Open the install guide <span aria-hidden="true">↗</span></a></div>
          <div className="install-card"><div className="install-card-title"><Mark small /><span>A little setup. A lot less typing.</span></div><InstallCommand /><div className="install-notes"><p>Apple Silicon · macOS 14 or later</p><p>No paid Apple Developer membership needed for a local build. Approve Apple’s tools installer if prompted; setup continues automatically. You also approve macOS privacy permissions.</p><a href={`${repo}/blob/main/scripts/bootstrap.sh`}>View the setup script <span aria-hidden="true">↗</span></a><br /><a href={`${repo}/blob/main/AGENTS.md`}>Installing with an agent? Read AGENTS.md <span aria-hidden="true">↗</span></a></div></div>
        </section>
        <section className="faq wrap" id="questions"><h2>A few things<br />you might wonder.</h2><div>
          ...
          <details><summary>Is there a download-and-open app?<span>+</span></summary><p>This release is a source-build preview. The setup command installs missing tools, builds the app on your Mac and opens it. Already have the source? Use Install Whisplet.command. A notarized download and automatic binary updates are not available yet.</p></details>
